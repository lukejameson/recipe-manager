<script lang="ts">
  ...
  function handleAdd() {
    if (selectedRecipe) {
      // Ensure servingsNeeded is always a number
      const numericServings = typeof servingsNeeded === 'number' ? servingsNeeded : parseFloat(servingsNeeded) || 1;
      onAdd(selectedRecipe, numericServings);
    }
  }
