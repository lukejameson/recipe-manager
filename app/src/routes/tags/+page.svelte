<script lang="ts">
  ...
  async function saveRename() {
    ...
    try {
      await apiClient.updateTag(editingTag.id, newName.trim());
      await loadTags();
      editingTag = null;
      newName = '';
    } catch (err: any) {
  ...
  async function saveMerge() {
    ...
    try {
      await apiClient.mergeTags(mergingTag.id, targetTagId);
      await loadTags();
      mergingTag = null;
      targetTagId = '';
    } catch (err: any) {
  ...
  async function handleCleanupOrphaned() {
    ...
    cleaningUp = true;
    try {
      const result = await apiClient.cleanupOrphanedTags();
