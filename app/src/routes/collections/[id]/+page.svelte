<script lang="ts">
  ...
  async function loadCollection() {
    ...
    error = '';
    try {
      collection = await apiClient.getCollection($page.params.id);
    } catch (err: any) {
  ...
  async function handleAddRecipe(recipeId: string) {
    try {
      await apiClient.addRecipeToCollection($page.params.id, recipeId);
      loadCollection();
  ...
  async function handleRemoveRecipe(recipeId: string) {
    ...
    try {
      await apiClient.removeRecipeFromCollection($page.params.id, recipeId);
      loadCollection();
    } catch (err: any) {
