<script lang="ts">
  ...
  async function loadRecipes() {
    try {
      allRecipes = await apiClient.getRecipes();
    } catch (err: any) {
  ...
  async function handleAddItem(e: Event) {
    ...
    try {
      await apiClient.addShoppingListItem({
  ...
  async function handleGenerateFromRecipes() {
    ...
    try {
      await apiClient.generateShoppingListFromRecipes({
  ...
  async function handleToggleChecked(id: string) {
    try {
      await apiClient.toggleShoppingListItem(id);
      loadItems();
    } catch (err: any) {
  ...
  async function handleDeleteItem(id: string) {
    try {
      await apiClient.deleteShoppingListItem(id);
      loadItems();
    } catch (err: any) {
  ...
  async function handleClearChecked() {
    ...
    try {
      await apiClient.clearShoppingList(true);
      loadItems();
    } catch (err: any) {
