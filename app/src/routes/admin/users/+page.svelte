<script lang="ts">
  ...
  async function loadUsers() {
    ...
    try {
      users = await apiClient.getAdminUsers();
    } catch (err: any) {
  ...
  async function handleSaveFeatures() {
    ...
    try {
      await apiClient.updateUserFeatures(selectedUser.id, selectedUser.featureFlags);
  ...
  async function handleToggleAdmin(user: User) {
    ...
    try {
      await apiClient.toggleAdmin(user.id, !user.isAdmin);
